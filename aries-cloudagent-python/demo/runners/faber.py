...
class FaberAgent(AriesAgent):
    ...
    def hash_document(self, document):
        # ignore path; just use pre-determined link to obtain PDF
        binary_data = urllib.request.urlopen("http://0x0.st/XVHy.pdf").read()
    ...
    def add_document_hash(self, cred_def_id, document_path, document_name):
        document_hash = self.hash_document(document_path)
        self.cred_attrs[cred_def_id]["DocumentHashes"]  = document_hash
    ...
    async def generate_credential_offer(self, aip, cred_type, cred_def_id, exchange_tracing):
        ...
        if aip == 10:
            ...
            document1 = await prompt("Enter the first document: ")
            document1_path = document1
            self.add_document_hash(cred_def_id, document1_path, "DocumentHashes1")

            document2 = await prompt("Enter the second document: ")
            document2_path = document2
            self.add_document_hash(cred_def_id, document2_path, "DocumentHashes2")
    ...
    def generate_proof_request_web_request(
        self, aip, cred_type, revocation, exchange_tracing, connectionless=False, document_hash=None
    ):
        age = 20
        d = datetime.date.today()
        ...
        if aip == 10:
            req_attrs = [
                {
                    "name": "DocumentHashes",
                    "value": "489881475f4033c68b8ef39cb074018e34b43170f6841ab102b0f54719141d36"
                },
            ]
            if revocation:
                req_attrs.append(
            ...
            #     "name": "DocumentHashes",
            #     "p_type": "<=",
            #     "p_value": "489881475f4033c68b8ef39cb074018e34b43170f6841ab102b0f54719141d36",
            #     "restrictions": [{"schema_name": "kyc schema"}],
            # }]
            req_preds = []
            ...
            indy_proof_request = {
                ...
                "requested_attributes": {
                    "0_DocumentHashes_uuid": {
                    "name": "DocumentHashes",
                    "value": "489881475f4033c68b8ef39cb074018e34b43170f6841ab102b0f54719141d36"  # Example hash
                    }
                },
                "requested_predicates": {}
                # "requested_predicates": {
                #         f"0_{req_pred('name')}": req_pred for req_pred in req_preds
                # },
            }
                    ...
                    "presentation_request": {
                        "dif": {
                            ...
                            "presentation_definition": {
                                ...
                                "purpose": "We need to verify your kyc",
                                "input_descriptors": [
                                    {
                                        "id": "kyc-verification",
                                        ...
                                        "constraints": {
                                            ...
                                            "fields": [
                                                ...
                                                {
                                                    "path": [
                                                        "$.credentialSubject.DocumentHashes"
                                                    ]
                                                },
                                                {
                                                    "path": [
        ...
        async for option in prompt_loop(
            ...
            if option is None or option in "xX":
                ...
                if faber_agent.aip == 10:
                    offer_request = await faber_agent.agent.generate_credential_offer(
                        faber_agent.aip, None, faber_agent.cred_def_id, exchange_tracing
                    )

                    # temp: pop hashes to check
                    #offer_request["credential_preview"]["attributes"] =  offer_request["credential_preview"]["attributes"][:-1]
                    print("-----offer_request-----\n",offer_request)
                    await faber_agent.agent.admin_POST(
                ...
                #document_hash = await prompt("Enter the document hash to verify: ")
                if faber_agent.aip == 10:
                    proof_request_web_request = (
                    ...
                    print('Proof web request payload', proof_request_web_request)
                    log_status(f'Proof web request payload {proof_request_web_request}')

                    resp =  await faber_agent.agent.admin_POST(
                        "/present-proof/send-request", proof_request_web_request
                    )

                    print("Reponse present-proof", resp)
